...
def get_bb(num_buffers=64, batch_size=1, start_with_guide=True, see_traces=True):
    ...
    for i in range(num_buffers):
        ...
        if start_with_guide and (i != num_buffers - 1):
        ...
        bb.append(buff)
        #print(buff.traces)
    score = average_return(bb[-1:]) # make 1 buffer 'natural' and the others fake; use natural as the baseline expected return
...
def train_policy(policy_optimizer, epochs, buffer_buffer, policy_clip_range=0.1, entropy_loss_weight=0.01, ignore_last=True):
...
#    get_value.eval()
    ...
    print(f"Return before training was {average_return(buffer_buffer[-1:])}")
    #if round_num > 0:
    print("\n~~~~~~~POLICY loop~~~~~~~\n")
    train_policy(policy_optimizer, policy_epochs, buffer_buffer, policy_clip_range, entropy_loss_weight)
    #print("\n~~~~~~~~VALUE loop~~~~~~~~~~~\n")
...
for i in range(num_rounds):
    ...
    if ((i + 1) in [1, 2, 3, 4, 5, 8, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000]) or (((i + 1) > 10000) and ((i + 1) % 10000 == 0)):
        torch.save(brain.state_dict(), f'brain_checkpoints/brain_EXPERIMENTAL_5output_weights_fake-traces_RL_GRPO_v1_round{i + 1}.pth')
